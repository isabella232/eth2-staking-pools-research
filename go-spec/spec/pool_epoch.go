...
func (summary *PoolExecutionSummary) ApplyOnState(state *State) error {
	...
	for duty, whoExecuted := range summary.Performance {
		switch duty.Type {
		case 0: // attestation
			for i:=0 ; i < int(PoolExecutorsNumber) ; i++ {
				executor := pool.SortedExecutors[i]
				if !duty.Included {
					_,err := state.DecreaseBlockProducerBalance(executor, 2*BaseEth2DutyReward)
					if err != nil {
						return err
					}
				} else {
					if IsBitSet(whoExecuted[:], uint64(i)) {
						_,err := state.IncreaseBlockProducerBalance(executor, BaseEth2DutyReward)
						if err != nil {
							return err
						}
					} else {
						_,err = state.DecreaseBlockProducerBalance(executor, BaseEth2DutyReward)
						if err != nil {
							return err
						}
					}
		...
		case 1: // proposal
			for i:=0 ; i < int(PoolExecutorsNumber) ; i++ {
				executor := pool.SortedExecutors[i]
				if !duty.Included {
					_,err = state.DecreaseBlockProducerBalance(executor, 4*BaseEth2DutyReward)
					if err != nil {
						return err
					}
				} else {
					if IsBitSet(whoExecuted[:], uint64(i)) {
						_,err = state.IncreaseBlockProducerBalance(executor, 2*BaseEth2DutyReward)
						if err != nil {
							return err
						}
					} else {
