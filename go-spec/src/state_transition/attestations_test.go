	...
	for _, test := range tests {
		t.Run(test.name, func (t *testing.T) {
			...
			if test.expectedError != nil {
				require.EqualError(t, st.processBlockAttestations(state, test.block.Body.Attestations), test.expectedError.Error())
			} else {
				require.NoError(t, st.processBlockAttestations(state, test.block.Body.Attestations))
			}
		})
	}
}